...
############################################################################
#
# ANELLO FW upgrade script
# contact info@anellophotonics.com for support
#
###########################################################################
...
class MavlinkSerialPort():
    ...
    def __init__(self, portname, baudrate=0, devnum=0, debug=0):
        self.baudrate = 0
        self._debug = debug
        self.buf = ''
        self.port = devnum
...
class uploader(object):
    '''Uploads a firmware file to the bootloader'''
...
def main():
    ...
    if os.path.exists("/usr/sbin/ModemManager"):
        print("==========================================================================================================")
        print("WARNING: You should uninstall ModemManager as it conflicts with any non-modem serial device")
        print("==========================================================================================================")
    ...
    try:
        while True:
            ...
            for port in portlist:
                ...
                # Try reboot into bootloader through RS-232-1/MAVLink.
                print("Connecting to RS-232-1 MAVLink...")
                mav_serialport = MavlinkSerialPort(port, baudrate=baud_flightstack[0], devnum=10)

                # Wait for the heartbeat msg to confirm MAVLink is running
                print("Waiting for heartbeat...")
                heartbeat = mav_serialport.mav.wait_heartbeat(timeout=5)

                if heartbeat is None:
                    # No heartbeat, close MAVLink.
                    print("No heartbeat from MAVLink, trying NMEA0183...")
                    mav_serialport.mav.close()
                    time.sleep(0.5)

                    # Try reboot into bootloader through RS-232-1/NMEA0183.
                    print("Connecting to RS-232-1 NMEA0183...")
                    test_serial_port = serial.Serial(port, baudrate=baud_flightstack[0])
                    bytes = b'$APRBL,1*50'
                    test_serial_port.write(bytes)
                    time.sleep(0.5)
                    test_serial_port.close()
                    print("Waiting for bootloader...")
                else:
                    print("Heartbeat received!")
                    mav_serialport.write("reboot -b\n")
                    time.sleep(0.5)
                    mav_serialport.mav.close()
                    print("Waiting for bootloader...")
